...
// The hero is a fixed 560px tall at lg+ (see FeaturedHero's lg:h-[560px]);
// the 3 stacked side cards need to sum to exactly that (560 = 3 cards + 2
// gap-4 gaps -> 176px each, see FeaturedSideCard's lg:h-[176px]).
...
function FeaturedBadge() {
  return (
    <span className="inline-block bg-red-600 px-2 py-1 text-xs font-bold uppercase tracking-wide text-white">
      Featured
    </span>
  );
}
...
function FeaturedHero({ article }: { article: ArticleSummary }) {
  return (
    <Link
      href={`/${article.slug}`}
      className="group relative block aspect-[4/3] overflow-hidden bg-neutral-900 sm:aspect-[16/11] lg:aspect-auto lg:h-[560px]"
    >
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-3 p-6">
        <FeaturedBadge />
        <h2 className="text-2xl font-extrabold leading-tight text-white sm:text-3xl">{article.title}</h2>
...
function FeaturedSideCard({ article }: { article: ArticleSummary }) {
  return (
    <Link
      href={`/${article.slug}`}
      className="group relative flex h-full min-h-[110px] overflow-hidden bg-neutral-900 lg:h-[176px] lg:min-h-0"
    >
      {article.featuredImage ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img
      ...
      <div className="absolute inset-x-0 bottom-0 flex flex-col gap-1.5 p-4">
        ...
        <h3 className="text-sm font-bold leading-snug text-white line-clamp-2">{article.title}</h3>
