...
// the most recently published articles as an honest stand-in rather than
// fabricating a trending signal.
export async function TrendingBar() {
  const { articles } = await api.listArticles({ pageSize: 6 });
  if (articles.length === 0) return null;

  const items = articles.map((article) => ({ id: article.id, slug: article.slug, title: article.title }));
